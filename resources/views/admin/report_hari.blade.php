...
<div class="py-14 px-6">
    <div class="mb-10">
        <h1 class="text-caption text-2xl font-bold m">PENJUALAN HARIAN</h1>
    </div>

    <div class="flex justify-center h-24 mb-4 items-center w-full bg-fill rounded-xl">
        <form class="flex justify-beetwen space-x-6 items-center">
            <div>
                <input class="border border-primary py-1 px-96 rounded-3xl text-caption font-bold" type="date" name="q" value="{{ $q }}">
            </div>
            <div>
    ...
    <div class="flex justify-center items-center h-screen w-full bg-fill rounded-xl">
            ...
            <div class="w-full mb-6">
                <div class="flex space-x-7 mb-7">
                    <div class="text-fill bg-primary w-1/2 h-40 rounded-xl px-8 py-6">
                        ...
                        @if($dataDetail != [])<h1 class="font-bold mt-6 text-5xl">{{ $banyakMenu->banyak }}</h1>@else<h1 class="text-fill font-bold text-2xl">Tidak ada Pemesanan</h1>@endif
                    </div>
                    <div class="text-fill bg-primary w-1/2 h-40 rounded-xl px-8 py-6">
                        <p>Jumlah Menu Yang Terjual</p>
                        @if($dataDetail != [])<h1 class="font-bold mt-6 text-5xl">{{ $jumlahMenu->jumlah }}</h1>@else<h1 class="text-fill font-bold text-2xl">Tidak ada Pemesanan</h1>@endif
                    </div>
                </div>
                <div class="text-fill bg-primary w-full h-40 rounded-xl px-8 py-6">
                    <p>Total Pemesanan Yang dibuat</p>
                    @if($dataDetail != [])<h1 class="font-bold mt-6 text-5xl"> {{$totalPesanan}} </h1>@else<h1 class="text-fill font-bold text-2xl">Tidak ada Pemesanan</h1>@endif
                </div>
            </div>
